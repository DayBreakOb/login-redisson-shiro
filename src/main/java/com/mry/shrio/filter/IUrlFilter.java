	...
	public Map<String, String> decodeParams(ServletRequest request, ServletResponse response) {
		...
		try {
			...
			if (strs.length == 3) {
				...
				System.out.println(interval + "__________________");
				if (interval > BaseConfig.requestMaxLogin || interval < BaseConfig.requestMinLogin) {
					ServletUtils.renderString(WebUtils.toHttp(response), "Illegal request is forbidden!!!",
							"text/html");
		...
		HttpServletRequest req = WebUtils.toHttp(request);
		if (!ServletUtils.handleExcludeUrl(req, excludes)) {
			map.put("forbidden", "true");
		}
		return map;
	}
